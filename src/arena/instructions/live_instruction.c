...
static void write_player_line(robot_info_t *robot, arena_t *arena)
{
    ...
    asprintf(&line, "The player %d(%s) is alive.", robot->prog_num,
             robot->header->prog_name);
    if (BONUS_MODE == 0)
    {
        puts(line);
    }
...
static bool find_process(process_data_t **processes, byte4_t process_count,
                         sbyte4_t prog_num, arena_t *arena)
{
