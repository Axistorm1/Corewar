...
static byte4_t get_param(char *param)
{
    return (byte4_t)(((param[0] - '0') * 10) + (param[1] - '0'));
}

// Can be used to check if the param value is correct for error handling
static void get_values(u_char *bin, int type, instruction_t *instruction, int i)
{
...
static int store_params(instruction_t *instruction,
                        unsigned int type_arr[MAX_ARGS], u_char *bin)
{
    int idx = 0;
