...
static char *get_parameter_type(instruction_t *instruction, int i)
{
...
static char *get_parameters(instruction_t *instruction, char *line_buffer)
{
    size_t len = 0;
    ...
    for (int i = 0; i != MAX_ARGS; i++)
    {
        ...
        if (instruction->types[i] == PARAM_INDEX ||
            instruction->types[i] == PARAM_DIRDEX)
...
static char *get_instruction_name(instruction_t *instruction)
{
    char *instruct_name[] = {"live", "ld",   "st",    "add", "sub", "and",
...
void write_dot_s_to_file(instruction_t **instructions, char *filename)
{
