...
typedef enum PACKED op_codes_e
{
    NONE,
    LIVE,
    LD,
...
typedef enum PACKED param_type_e
{
    PARAM_NOTHING = 0,
    PARAM_REG     = 1,
    PARAM_DIR     = 2,
...
typedef union PACKED parameter_u
{
    sbyte4_t dir;
    sbyte2_t index;
    byte2_t  ind;
...
typedef struct type_and_param_s
{
    param_type_t type;
    parameter_t  param;
} type_and_param_t;
...
typedef struct PACKED robot_info_s
{
    char     *filename;
    header_t *header;
    byte1_t  *memory;
...
typedef struct PACKED gap_placement_info_s
{
...
typedef struct PACKED process_data_s
{
